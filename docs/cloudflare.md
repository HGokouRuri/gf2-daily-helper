...
2. `git clone` 本项目，在项目目录下 `pnpm install` 安装依赖
3. `pnpm run deploy`，会弹出一个页面要求授权，完成操作之后就完成了 cloudflare worker 的部署
4. 登录到 [Cloudflare 控制台](https://dash.cloudflare.com/)，选择刚才部署的 worker，如下图
   ![1](./assets/cloudflare-1.png)
5. 添加三个环境变量 `ACCOUNT_NAME`、`PASSWORD` 和 `ENCRYPTION_KEY`，填写自己的散爆账号、密码和登录加密密钥，含义见 [通用配置说明](./configuration.md)，完成后效果如下图
